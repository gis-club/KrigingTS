...
export var kriging = (function () {
  var kriging = {}

  const createArrayWithValues = function (value, n) {
    var array = []
    for (var i = 0; i < n; i++) {
      array.push(value)
    }
  ...
  const kriging_variogram_gaussian = function (h, nugget, range, sill, A) {
    return (
      nugget + ((sill - nugget) / range) * (1.0 - Math.exp(-(1.0 / A) * Math.pow(h / range, 2)))
    )
  }
   ...
   */
  const FastPip = function (width, height, polygon) {
  ...
  kriging.train = function (t, x, y, model, sigma2, alpha) {
    ...
    for (i = 0; i < n; i++) {
      switch (model) {
        ...
        case 'spherical':
          X[i * 2 + 1] =
            1.5 * (lag[i] / variogram.range) - 0.5 * Math.pow(lag[i] / variogram.range, 3)
          break
      }
      Y[i] = semi[i]
    }
    ...
    for (i = 0; i < n; i++) {
      ...
      for (j = a[0]; j <= a[1]; j++)
        for (k = b[0]; k <= b[1]; k++) {
          xtarget = xlim[0] + j * width
          ytarget = ylim[0] + k * width
          if (fastPip(j, k)) A[j][k] = kriging.predict(xtarget, ytarget, variogram)
        }
    }
    A.xlim = xlim
    A.ylim = ylim
